...
export default async function BlogPostPage({ params }) {
  ...
  const options = {
    ...
    renderNode: {
      ...
      [BLOCKS.HEADING_1]: (node, children) => (
        <h1 className="text-4xl font-extrabold mb-6 mt-10">{children}</h1>
      ),
      ...
      [BLOCKS.EMBEDDED_ASSET]: (node) => {
        const asset = blogPost?.linkedAssets?.find(
        ...
        if (asset && typeof asset.url === 'string' && asset.url.length > 0) {
          return (
            <div className="my-8">
              ...
              />
              {asset.description && (
                <p className="text-center text-sm text-gray-500 mt-2">{asset.description}</p>
              )}
            </div>
          );
        }
      ...
      [BLOCKS.EMBEDDED_ENTRY]: (node) => {
        const entry = blogPost?.linkedEntries?.find(
            ...
            return (
              <div className="p-4 border rounded-lg bg-gray-50 my-6">
                ...
                {entry.profilePictureUrl &&
                  ...
                  entry.profilePictureUrl.length > 0 && (
                    <img
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4">
      <h1 className="text-3xl sm:text-5xl font-extrabold mb-6 text-gray-900 dark:text-white">
        {blogPost.title}
      </h1>
      {blogPost.author && (
      ...
      {blogPost.featuredImageUrl &&
        typeof blogPost.featuredImageUrl === 'string' &&
        blogPost.featuredImageUrl.length > 0 && (
          <div className="mb-8">
            {blogPost.featuredImageUrl && (
              <Image
