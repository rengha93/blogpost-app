...
export default async function BlogPost() {
  ...
  const response = await client
    .getEntries({
      ...
      select: [
        'fields.title,fields.slug,fields.excerpt,fields.featuredImage,fields.tag,fields.category,fields.author,fields.publishedDate,fields.timeToRead',
      ],
      include: 10,
    })
    .catch((error) => {
            ...
            <div key={post.id} className="py-4 my-5 border-b border-gray-10">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="col-span-3">
                  ...
                  <p className="text-sm md:text-md text-neutral-500 dark:text-gray-200 py-0.5 sm:py-2">
                    {post.excerpt}
                  </p>
                  {post.tag && (
                    <p className="inline-block bg-gray-100 mt-2 text-gray-800 dark:bg-gray-700 dark:text-gray-200 text-xs font-semibold px-2.5 py-0.5 rounded-full border border-gray-200">
                      {post.tag.name}
                    </p>
                  )}
                </div>
